...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
    ...
    if (rightIndex < last) {
        // Determine the smallest child if two children exist
        if (compare(dyGet(heap, leftIndex), dyGet(heap, rightIndex)) == -1) {
            smallestChild = leftIndex;
        } else {
            smallestChild = rightIndex;
        }

        // Adjust heap recursively towards the root
        ...
        // Only 1 child; compare and adjust towards the root
        if (compare(dyGet(heap, leftIndex), dyGet(heap, position)) == -1) {
...
void dyHeapAdd(DynamicArray* heap, TYPE value, compareFunction compare)
{
    ...
    TYPE pValue = heap->data[parent];

    // Swap current/parent if current is less than the parent
    while (current != 0 && compare(value, pValue) == -1) {
        dySwap(heap, current, parent);
        current = parent;
        parent = (current - 1) / 2;
        pValue = heap->data[parent];
    }
}
...
void dyHeapRemoveMin(DynamicArray* heap, compareFunction compare)
{
    ...
    // Copy the last element to index 0; remove last index.
...
void dyHeapSort(DynamicArray* heap, compareFunction compare)
{
    ...
    // Swap less-than elements and adjust towards the root; decrement each iteration.
